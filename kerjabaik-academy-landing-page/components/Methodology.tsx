...
const MethodCard: React.FC<CardProps> = ({ icon, title, description, delay = 0 }) => (
  <ScrollReveal direction="up" delay={delay}>
    <div className="flex flex-col items-center text-center p-6 transition-all duration-300 group hover:-translate-y-2">
      <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-50 text-primary transition-all duration-300 group-hover:bg-primary group-hover:text-white dark:bg-slate-800 group-hover:shadow-lg group-hover:shadow-primary/30">
        <span className="material-symbols-outlined text-3xl">{icon}</span>
      </div>
      <h3 className="mb-3 text-lg font-bold text-text-main dark:text-white">{title}</h3>
      <p className="text-sm leading-relaxed text-text-sub dark:text-slate-400">{description}</p>
    </div>
  </ScrollReveal>
);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal direction="none">
          <div className="text-center mb-12">
            <h2 className="text-sm font-bold text-primary uppercase tracking-[0.2em] mb-3">Metodologi Belajar Kami</h2>
