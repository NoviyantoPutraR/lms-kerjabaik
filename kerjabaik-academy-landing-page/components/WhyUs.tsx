...
const FeatureItem: React.FC<FeatureItemProps> = ({ icon, title, description, delay = 0 }) => (
  <ScrollReveal direction="left" delay={delay}>
    <div className="flex gap-5 group">
      ...
      <div>
        <h3 className="text-xl font-bold text-text-main dark:text-white group-hover:text-primary transition-colors">{title}</h3>
        <p className="mt-2 text-text-sub dark:text-slate-400 leading-relaxed text-sm md:text-base">{description}</p>
      </div>
    </div>
  </ScrollReveal>
);
        ...
        <div className="grid gap-16 lg:grid-cols-2 items-center">
          <ScrollReveal direction="right" className="order-2 lg:order-1 relative">
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-6 pt-12">
                <div 
                  className="h-56 w-full rounded-2xl bg-slate-100 bg-cover bg-center shadow-lg dark:bg-slate-800 transform hover:rotate-2 transition-transform duration-500" 
                  style={{ backgroundImage: `url('${IMAGES.whyUs[0]}')` }}
                ></div>
                <div 
                  className="h-72 w-full rounded-2xl bg-slate-100 bg-cover bg-center shadow-lg dark:bg-slate-800 transform hover:-rotate-1 transition-transform duration-500" 
                  style={{ backgroundImage: `url('${IMAGES.whyUs[1]}')` }}
                ></div>
              </div>
              <div className="space-y-6">
                <div 
                  className="h-72 w-full rounded-2xl bg-slate-100 bg-cover bg-center shadow-lg dark:bg-slate-800 transform hover:-rotate-2 transition-transform duration-500" 
                  style={{ backgroundImage: `url('${IMAGES.whyUs[2]}')` }}
                ></div>
                <div 
                  className="h-56 w-full rounded-2xl bg-slate-100 bg-cover bg-center shadow-lg dark:bg-slate-800 transform hover:rotate-1 transition-transform duration-500" 
                  style={{ backgroundImage: `url('${IMAGES.whyUs[3]}')` }}
                ></div>
              </div>
            </div>
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform rounded-3xl bg-primary p-6 shadow-2xl shadow-primary/40 animate-bounce-subtle">
          ...
          <div className="order-1 lg:order-2">
            <ScrollReveal direction="left" delay={100}>
              <h2 className="text-4xl font-extrabold tracking-tight text-text-main dark:text-white mb-6">Mengapa Profesional Memilih Kerjabaik Academy?</h2>
              <p className="text-lg text-text-sub dark:text-slate-400 mb-12">Kami fokus pada hasil nyata. Kurikulum kami disusun berdasarkan kebutuhan pasar kerja global saat ini.</p>
            </ScrollReveal>
