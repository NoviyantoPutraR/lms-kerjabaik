...
export function Timer() {
    ...
    return (
        <div className='flex flex-col items-center py-4 gap-2'>
            <p className='text-xs text-gray-500'>{time === 0 ? 'Menunggu' : 'Berjalan'}</p>

            <p className='text-gray-800 text-2xl font-semibold font-mono'>
                {displayTime.split(':')[0]} :
                {' '}{displayTime.split(':')[1]} :
                {' '}
            ...
            <button onClick={() => setIsPaused(!isPaused)} className='text-primary relative text-xs flex font-medium items-center gap-1 hover:text-primary/80 transition-colors'>
                ...
                <span className="ml-5">
                    {isPaused ? 'Mulai' : 'Jeda'} Pelacakan Waktu
                </span>
            </button>
        </div>
    )
