...
const CourseCard: React.FC<CourseCardProps> = ({ 
  ...
  <ScrollReveal direction="up" delay={delay}>
    <div className="group relative flex flex-col bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-800 transition-all hover:shadow-xl hover:border-primary-landing/20 h-full">
      ...
      <div className="px-2 pb-2 flex flex-col flex-1">
        <h3 className="mb-3 text-2xl font-bold leading-tight text-text-main dark:text-white group-hover:text-primary-landing transition-colors font-lexend">
          {title}
        </h3>
        <p className="mb-6 text-sm text-text-sub dark:text-slate-400 line-clamp-2 leading-relaxed font-lexend">
          {description}
        </p>
        <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-4 dark:border-slate-800/50">
          ...
          <span className={`text-lg font-black font-lexend ${isFree ? 'text-green-600' : 'text-primary-landing'}`}>
            {price}
          </span>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-16 flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          <ScrollReveal direction="right" className="max-w-2xl">
            <h2 className="text-4xl font-extrabold tracking-tight text-text-main dark:text-white font-lexend">Kursus Profesional Populer</h2>
            <p className="mt-4 text-lg text-text-sub dark:text-slate-400 font-lexend">Jelajahi kurikulum terbaik yang dirancang khusus untuk membantu kemajuan karier Anda di industri modern.</p>
          </ScrollReveal>
          <ScrollReveal direction="left" delay={200}>
            <Link className="group flex items-center font-bold text-primary-landing transition-all hover:gap-2 font-lexend" to="/login">
              Lihat Semua Kursus
