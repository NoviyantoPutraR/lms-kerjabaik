...
const MethodCard: React.FC<CardProps> = ({ icon, title, description, delay = 0 }) => (
  <ScrollReveal direction="up" delay={delay}>
    <div className="flex flex-col items-center text-center p-6 transition-all duration-300 group hover:-translate-y-2">
      ...
      <h3 className="mb-2 text-base font-bold text-text-main dark:text-white font-lexend">{title}</h3>
      <p className="text-xs lg:text-sm leading-relaxed text-text-sub dark:text-slate-400 font-lexend">{description}</p>
    </div>
  </ScrollReveal>
);
      ...
      <div className="mx-auto max-w-6xl px-6 sm:px-12 lg:px-16">
        <ScrollReveal direction="none">
          <div className="text-center mb-12">
            <h2 className="text-xs font-bold text-primary-landing uppercase tracking-[0.2em] mb-3 font-lexend">Metodologi Belajar Kami</h2>
